...
const FULL_TO_ALIAS = { // power alias
  "Egg Power": "Egg",
...
function getNumberOfPlayers(ingredients) {
...
function getMaxTotalPieces(fillings) {
  let ret = 0;
...
function calculateLevels(types, sandwich) {
...
function calculateTypes(baseTypes, sandwich) {
  const stars = sandwich.stars;
...
function craftSandwich(fillings, condiments, sums, presetSandwich) {
...
function isOneTwoSandwich(sandwich) {
  return oneTwoFirst.indexOf(sandwich.number) !== -1;
};

function hasOneHerba(condiments) {
  let count = 0;
...
function hasTwoHerba(condiments) {
  let count = 0;
  for (const condiment of condiments) {
    if (condiment.name.toLowerCase().indexOf("herba") !== -1) {
      count++;
...
function hasRelevance(ingredient, key) {
...
function mode(array, prop, skipMax = false, useMin = false) {
...
function copyTextToClipboard(text, postFunc) {
...
// Version 4.0
function shadeColor(color, percent = 30) {
...
module.exports = { getIngredientsFromRecipe, getIngredientsSums, checkPresetSandwich, getCondiments, getFillings, getRecipeFromIngredients, getNumberOfPlayers, ALIAS_TO_FULL, FULL_TO_ALIAS, COLORS, shadeColor, oneTwoFirst, craftSandwich, copyTextToClipboard, hasRelevance, getCategory, isType, isPower, isFlavor, isFilling, mode, FLAVOR_TABLE_EZ, calculatePowerAmount};
